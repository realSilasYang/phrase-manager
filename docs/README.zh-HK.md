...
常用語管理是一款運行於 uTools 的本地文字管理插件，使用「分類 → 分組 → 常用語」整理客服回覆、工作範本、提示詞、程式碼片段及筆記。資料預設保存在 uTools 本地資料庫，可匯出 JSON 備份。

## 功能

- 建立、重新命名、刪除及排序分類、分組和常用語
...
1. 從 [最新 Release](https://github.com/realSilasYang/phrase-manager/releases/latest) 下載並解壓插件包。
2. 在 uTools 開發者工具載入解壓目錄內的 `plugin.json`。
3. 在 uTools 搜尋「常用語」或 `Phrase Manager` 開啟插件。
4. 點擊「新建分類」或下拉選單中的「新建分類…」開始整理；分類是最上層，每個分類包含分組，建立時可輸入自訂名稱。

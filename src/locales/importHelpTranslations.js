...
export default {
  ...
  "zh-HK": {
    ...
    help: {
      ...
      importJsonTitle: "標準備份 JSON", importJsonDesc: "標準備份 JSON 的根物件必須包含「分类」「分组」「常用语」三個陣列；群組透過「所属分类编号」歸屬分類，常用語透過「所属分组编号」歸屬群組，具體結構見下方例子。",
      ...
      importReviewTitle: "預覽與匯入方式", importReviewDesc: "預覽會統計分類、群組、常用語、重複項目和無效列。你可選擇合併、保留複本或覆蓋；覆蓋會取代目前所有資料，請先核對統計。",
  ...
  "zh-TW": {
    ...
    help: {
      ...
      importJsonTitle: "標準備份 JSON", importJsonDesc: "標準備份 JSON 的根物件必須包含「分类」「分组」「常用语」三個陣列；群組透過「所属分类编号」歸屬分類，常用語透過「所属分组编号」歸屬群組，完整結構請參考下方範例。",
      ...
      importReviewTitle: "預覽與匯入方式", importReviewDesc: "預覽會統計分類、群組、常用語、重複項目和無效列。可選擇合併、保留副本或覆蓋；覆蓋會取代目前全部資料，請先核對統計。",
      importLimits: "文字檔支援 UTF-8 和 GBK。AI 轉換最多讀取 60,000 個字元；不支援 XLS/XLSX、其他二進位檔案或直接貼上文字。AI 匯入提示詞可在設定中修改。"
    },
  ...
  en: {
    ...
    help: {
      ...
      importJsonTitle: "Standard backup JSON", importJsonDesc: "The root must contain the Chinese arrays “分类”, “分组”, and “常用语”. Groups belong to categories through “所属分类编号”, and phrases belong to groups through “所属分组编号”, as shown below.",
      ...
      importReviewTitle: "Preview and import method", importReviewDesc: "The preview counts categories, groups, phrases, duplicates, and invalid rows. Choose Merge, Keep copies, or Replace all. Replace all removes the current data, so review the counts first.",
  ...
  ja: {
    ...
    help: {
      ...
      importJsonTitle: "標準バックアップJSON", importJsonDesc: "ルートには中国語キーの配列「分类」「分组」「常用语」が必要です。グループは「所属分类编号」でカテゴリに、定型文は「所属分组编号」でグループに関連付けます。具体的な構造は次の例を参照してください。",
      ...
      importReviewTitle: "プレビューと取り込み方法", importReviewDesc: "カテゴリ、グループ、定型文、重複、無効行の件数を確認できます。「統合」「コピーを保持」「すべて置換」から選択します。置換では現在の全データが入れ替わるため、先に件数を確認してください。",
  ...
  vi: {
    ...
    help: {
      ...
      importJsonTitle: "Bản sao lưu JSON chuẩn", importJsonDesc: "Đối tượng gốc phải có ba mảng dùng khóa tiếng Trung “分类”, “分组” và “常用语”. Nhóm thuộc danh mục qua “所属分类编号”, còn mẫu câu thuộc nhóm qua “所属分组编号” như trong ví dụ.",
      ...
      importReviewTitle: "Xem trước và cách nhập", importReviewDesc: "Bản xem trước thống kê danh mục, nhóm, mẫu câu, mục trùng lặp và dòng không hợp lệ. Chọn Hợp nhất, Giữ bản sao hoặc Thay thế tất cả. Thay thế sẽ đổi toàn bộ dữ liệu hiện có, vì vậy hãy kiểm tra số liệu trước.",
  ...
  ko: {
    ...
    help: {
      ...
      importJsonTitle: "표준 백업 JSON", importJsonDesc: "최상위 객체에는 중국어 키 배열 “分类”, “分组”, “常用语”가 있어야 합니다. 그룹은 “所属分类编号”으로 카테고리에, 상용구는 “所属分组编号”으로 그룹에 연결됩니다.",
      ...
      importReviewTitle: "미리보기와 가져오기 방법", importReviewDesc: "카테고리, 그룹, 상용구, 중복 항목, 유효하지 않은 행의 수를 확인합니다. 병합, 사본 유지, 모두 바꾸기 중에서 선택하세요. 모두 바꾸기는 현재 데이터를 전부 교체하므로 먼저 통계를 확인하세요.",
  ...
  es: {
    ...
    help: {
      ...
      importJsonTitle: "Copia de seguridad JSON estándar", importJsonDesc: "La raíz debe contener los arrays con claves chinas “分类”, “分组” y “常用语”. Los grupos pertenecen a una categoría mediante “所属分类编号” y las frases a un grupo mediante “所属分组编号”, como muestra el ejemplo.",
      ...
      importReviewTitle: "Vista previa y método de importación", importReviewDesc: "La vista previa cuenta categorías, grupos, frases, duplicados y filas no válidas. Elige Combinar, Conservar copias o Reemplazar todo. Reemplazar cambia todos los datos actuales; revisa antes las cifras.",
  ...
  fr: {
    ...
    help: {
      ...
      importJsonTitle: "Sauvegarde JSON standard", importJsonDesc: "La racine doit contenir les tableaux aux clés chinoises « 分类 », « 分组 » et « 常用语 ». Les groupes sont rattachés aux catégories par « 所属分类编号 » et les phrases aux groupes par « 所属分组编号 », comme dans l’exemple.",
      ...
      importReviewTitle: "Aperçu et mode d’importation", importReviewDesc: "L’aperçu compte les catégories, groupes, phrases, doublons et lignes non valides. Choisissez Fusionner, Conserver des copies ou Tout remplacer. Le remplacement écrase toutes les données actuelles : vérifiez d’abord les chiffres.",
  ...
  "pt-BR": {
    ...
    help: {
      ...
      importJsonTitle: "Backup JSON padrão", importJsonDesc: "A raiz deve conter os arrays com chaves chinesas “分类”, “分组” e “常用语”. Os grupos pertencem às categorias por “所属分类编号” e as frases aos grupos por “所属分组编号”, como mostra o exemplo.",
      ...
      importReviewTitle: "Prévia e modo de importação", importReviewDesc: "A prévia conta categorias, grupos, frases, duplicatas e linhas inválidas. Escolha Mesclar, Manter cópias ou Substituir tudo. A substituição troca todos os dados atuais; confira os números antes.",
  ...
  "pt-PT": {
    ...
    help: {
      ...
      importJsonTitle: "Cópia de segurança JSON padrão", importJsonDesc: "A raiz deve conter os arrays com chaves chinesas “分类”, “分组” e “常用语”. Os grupos pertencem às categorias por “所属分类编号” e as frases aos grupos por “所属分组编号”, como mostra o exemplo.",
      ...
      importReviewTitle: "Pré-visualização e modo de importação", importReviewDesc: "A pré-visualização conta categorias, grupos, frases, duplicados e linhas inválidas. Escolha Intercalar, Manter cópias ou Substituir tudo. A substituição troca todos os dados atuais; confirme primeiro os números.",
  ...
  ru: {
    ...
    help: {
      ...
      importJsonTitle: "Стандартная резервная копия JSON", importJsonDesc: "Корневой объект должен содержать массивы с китайскими ключами «分类», «分组» и «常用语». Группы относятся к категориям через «所属分类编号», а фразы к группам через «所属分组编号», как показано в примере.",
      ...
      importReviewTitle: "Предпросмотр и способ импорта", importReviewDesc: "В предпросмотре подсчитываются категории, группы, фразы, дубликаты и ошибочные строки. Можно выбрать объединение, сохранение копий или полную замену. При замене текущие данные будут удалены, поэтому сначала проверьте числа.",
  ...
  de: {
    ...
    help: {
      ...
      importJsonTitle: "Standardmäßige JSON-Sicherung", importJsonDesc: "Das Stammobjekt muss die Arrays mit den chinesischen Schlüsseln „分类“, „分组“ und „常用语“ enthalten. Gruppen gehören über „所属分类编号“ zu Kategorien und Textbausteine über „所属分组编号“ zu Gruppen, wie das Beispiel zeigt.",
      ...
      importReviewTitle: "Vorschau und Importmethode", importReviewDesc: "Die Vorschau zählt Kategorien, Gruppen, Textbausteine, Duplikate und ungültige Zeilen. Zur Auswahl stehen Zusammenführen, Kopien behalten und Alles ersetzen. Beim Ersetzen gehen die aktuellen Daten verloren; prüfe deshalb zuerst die Zahlen.",
  ...
  it: {
    ...
    help: {
      ...
      importJsonTitle: "Backup JSON standard", importJsonDesc: "La radice deve contenere gli array con chiavi cinesi “分类”, “分组” e “常用语”. I gruppi appartengono alle categorie tramite “所属分类编号” e le frasi ai gruppi tramite “所属分组编号”, come mostra l’esempio.",
      ...
      importReviewTitle: "Anteprima e metodo di importazione", importReviewDesc: "L’anteprima conta categorie, gruppi, frasi, duplicati e righe non valide. Scegli Unisci, Mantieni copie o Sostituisci tutto. La sostituzione rimuove tutti i dati correnti: controlla prima i conteggi.",

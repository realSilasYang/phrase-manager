export default {
  ...
  ai: {
    ...
    prompt: { optimizeSystem: '你是一名專業的文案編輯。請在不改變原意的前提下，讓使用者提供的文字更專業、清楚且有條理。只輸出修改後的內容，不要加入說明。', themeSystem: '你是一名專業的文案助手。使用者會提供一段現有內容作為主題，請依照該主題撰寫一段全新且專業的常用語。只輸出生成內容，不要加入說明。', themeUser: '主題內容：\n{content}\n\n請依照以上主題生成新內容。{extra}', extraRequirements: '\n其他要求：{prompt}', generateSystem: '你是一名專業的文案助手。請依照使用者的描述，撰寫簡潔且專業的常用語。只輸出生成內容，不要加入說明。', categorizeSystem: '你是一名分類助手。請依照內容，從以下分類及其群組中選擇最合適的分類與群組。\n結構：{structure}\n只輸出以下 JSON 格式，不要輸出其他內容：{"分类":"分類名稱","分组":"群組名稱"}', titleSystem: '你是一名標題助手。請依照內容產生簡短、準確的標題（不超過 15 個字）。只輸出標題，不要加入說明。' }
  },

...
左侧依次展示分类和分组，右侧展示当前分组中的常用语卡片。点击卡片可以复制内容，选中卡片后可编辑标题和正文；拖拽可以调整顺序，也可以在分类、分组之间移动或复制条目。顶部搜索框支持跨内容查找，底部工具栏提供设置、帮助、导入和导出入口。
...
常用语、分类、分组和设置通过 uTools 的本地数据库保存，不会写入项目目录，也不会因安装插件自动上传到远程服务。删除插件前建议先使用“导出”保存 JSON 备份。
...
```json
{
  ...
  "分组": [{ "编号": "group-1", "名称": "回复模板", "所属分组编号": "category-1" }],
  "常用语": [{
    "编号": "phrase-1",
    "标题": "欢迎语",
    "内容": "你好，感谢你的咨询。",
...
领域数据由三组集合组成：分类保存顶层顺序，分组通过 `所属分组编号` 兼容关联分类，常用语通过 `所属分类编号` 关联分组。每条记录都有稳定的 `编号`、名称或内容、排序和时间字段。新建、导入、AI 结果和拖拽都会经过统一规范化，避免重复编号、孤儿分组和越界归属。旧版备份字段保持兼容，升级时不会丢失已有数据。

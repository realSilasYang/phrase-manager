...
常用语管理（Phrase Manager）是一款运行在 uTools 中的本地文本管理插件。它使用“分类 → 分组 → 常用语”的结构整理可复用内容，适合管理客服回复、工作模板、提示词、代码片段和个人笔记。数据默认保存在 uTools 本地数据库，可通过 JSON 备份恢复；AI 功能用于生成内容、标题、分类和导入结构化数据。
...
左侧展示分组，中间展示当前分组中的常用语卡片，右侧展示分类。点击卡片可以复制内容，选中卡片后可编辑标题和正文；拖拽可以调整顺序，也可以在分类、分组之间移动或复制条目。顶部搜索框支持跨内容查找，底部工具栏提供设置、帮助、导入和导出入口。
...
4. 第一次使用时，可以按照引导创建示例数据，也可以直接点击“新建分类”开始整理。

### 💾 了解数据保存范围

分类、分组、常用语和设置通过 uTools 的本地数据库保存，不会写入项目目录，也不会因安装插件自动上传到远程服务。删除插件前建议先使用“导出”保存 JSON 备份。
...
搜索框会在当前数据范围内匹配标题、正文、分类和分组。点击搜索结果可以直接跳转到对应分类并选中常用语。悬停卡片后按 `Space` 可进入预览模式，再用上下方向键浏览相邻卡片。

### 📦 批量管理

点击搜索框旁的批量选择按钮进入批量模式。之后可以全选、删除、移动、导出或调整多个分组和常用语；批量操作作为一次完整操作写入撤销历史。

### 🖱️ 拖拽移动和复制

拖拽分类、分组或常用语可以调整同级顺序，也可以放到其他分类或分组。按住 `Ctrl` 拖拽会保留原条目并创建副本；普通拖拽会移动原条目。拖拽过程中会显示目标位置和移动方向，松开鼠标后才提交变更。

## 5. 导入与导出

### 📥 支持的导入格式

- **JSON 备份：** 用于完整备份和恢复分类、分组、常用语及排序信息。
...
```json
{
  "分类": [{ "编号": "category-1", "名称": "工作" }],
  "分组": [{ "编号": "group-1", "名称": "回复模板", "所属分类编号": "category-1" }],
  "常用语": [{
    "编号": "phrase-1",
    "标题": "欢迎语",
    "内容": "你好，感谢你的咨询。",
...
| AI 生成标题 | 根据正文生成简短标题 |
| AI 智能归类 | 在“分类 → 分组”结构中推荐已有分类和分组，并严格保持父子关系 |
| AI 智能导入 | 将非标准文本转换为“分类 → 分组 → 常用语”结构并先预览 |
...
领域数据由三组集合组成：分类保存顶层顺序，分组通过 `所属分类编号` 关联分类，常用语通过 `所属分组编号` 关联分组。每条记录都有稳定的 `编号`、名称或内容、排序和时间字段。新建、导入、AI 结果和拖拽都会经过统一规范化，避免重复编号、孤儿分组和越界归属。
...
1. 保持分类、分组、常用语的关联和持久化字段遵循规范结构；变更字段时同步更新校验、导入导出和恢复路径。
